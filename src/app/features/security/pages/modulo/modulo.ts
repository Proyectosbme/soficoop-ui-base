...
@Component({
  selector: 'app-modulo',
  standalone: true,
  imports: [CommonModule, ReactiveFormsModule, TableComponent, FormDialogComponent, SearchPanelComponent, DetailPanelComponent, ConfirmDialog, ToastModule, PageTitleComponent],
...
export class ModuloPage implements OnInit {
  ...
  async loadModulos(): Promise<void> {
    ...
    try {
      const data = await this.moduloService.obtenerTodas();
      this.modulos = data;
    } catch (error) {
  ...
  async onSubmit(): Promise<void> {
    ...
    try {
      if (isUpdate) {
        const moduloActualizado = await this.moduloService.actualizar(id, { nombre: nombreTrim });
        ...
        const moduloCreado = await this.moduloService.crear({ nombre: nombreTrim });
...
if (!nom) {
    ...
    try {
      const encontrado = await this.moduloService.buscarPorId(id);
  ...
  async remove(modulo: TableItem): Promise<void> {
    ...
    try {
      await this.moduloService.eliminar(parsedId);
      await this.loadModulos();
