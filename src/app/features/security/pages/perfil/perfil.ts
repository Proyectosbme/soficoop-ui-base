...
@Component({
  ...
  imports: [CommonModule, ReactiveFormsModule, ConfirmDialog, ToastModule, SearchPanelComponent, TableComponent, DetailPanelComponent, FormDialogComponent, PageTitleComponent, QuickNavComponent],
...
export class PerfilPage implements OnInit, OnDestroy {
  ...
  async buscarPorId(): Promise<void> {
    ...
    try {
      const perfil = await this.perfilService.buscarPorId(id);
  ...
  async onSubmit(): Promise<void> {
    ...
    try {
      if (isUpdate) {
        const actualizado = await this.perfilService.actualizar(id, { nombre: nombreTrim });
        ...
        const creado = await this.perfilService.crear({ nombre: nombreTrim });
  ...
  async remove(perfil: TableItem): Promise<void> {
    ...
    try {
      await this.perfilService.eliminar(idValue);
